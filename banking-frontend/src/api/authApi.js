...
const API_URL_transactions = 'http://localhost:8082/transactions';

export const createTransaction = async (userId, accountId, amount, type) => {
  console.log(userId, accountId, amount, type)
  const response = await axios.post(`${API_URL_transactions}`, { userId, accountId, amount, type });
  return response.data;
};